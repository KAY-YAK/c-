...
void Menu::push_back(char *pOption) {
	// Check for overflow
	//cout<<"prev count "<<this->count<<endl;
	if (this->count == this->max_list_size) {
		this->double_capacity();
	}
	//cout<<"after count: "<<this->count<<endl;
...
Text Menu::remove(int index) {
	...
	// Copy from this->option_list to new_option_list
	// Copy the object to be deleted
	temp = this->option_list[index - 1];
	cout << temp << "\n";
	cout << this->max_list_size << "\n";
	...
	for (int c = 0; c < (this->max_list_size); c++) {		// -1 because max_list_size if 4 then index is 0, 1, 2, 3
		...
		new_option_list[new_index] = this->option_list[c];
		new_index = new_index + 1;
		//cout << "This :" << this->option_list[c] << "\nNew_option_list :" << new_option_list[c]<< "\n";
	}
	...
	for (int c = 0; c <= (this->max_list_size - 1); c++) {	//This might cause problem just try copying the whole array
		cout << "2nd loop" << c << "|||" << new_option_list[c].isEmpty() << "\n";
		if (new_option_list[c].isEmpty()) {
			cout << "Break in second loop\n";
			break;
		}
		this->option_list[c] = new_option_list[c];
	}
...
void Menu::pop_back() {
	...
	if (this->isEmpty()) {
		...
		cout << "INPOP_BACK ELSE\n";
		this->remove(this->count);		//send count and reduce it inside remove
		cout << "INPOP_BACK successful return";
	}
}
...
const Text Menu::toString() {
	...
	// if (this->count==0 && this->top_text.isEmpty() && this->top_prompt.isEmpty() && this->bottom_text.isEmpty() && this->bottom_prompt.isEmpty() && this->isEmpty()){
	// 	// Only append literal
	// 	toStr.append("?->");
	// 	return toStr;
	// }
